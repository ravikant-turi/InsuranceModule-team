		...
		<h:form>
			...
			<div class="filter-links">
				<h:commandLink value="FAMILY"
					action="#{insuranceSubscribeController.setSelectedPlanType('FAMILY')}"
					styleClass="filter-button" />
				<h:commandLink value="SELF"
					action="#{insuranceSubscribeController.setSelectedPlanType('SELF')}"
					styleClass="filter-button" />
				<h:commandLink value="SENIOR"
					action="#{insuranceSubscribeController.setSelectedPlanType('SENIOR')}"
					styleClass="filter-button" />
				<h:commandLink value="CRITICAL_ILLNESS"
					action="#{insuranceSubscribeController.setSelectedPlanType('CRITICAL_ILLNESS')}"
					styleClass="filter-button" />
				<h:commandLink value="EPIDEMIC_PROTECT"
					action="#{insuranceSubscribeController.setSelectedPlanType('EPIDEMIC_PROTECT')}"
					styleClass="filter-button" />
				<h:commandLink value="SUPER_ELITE"
					action="#{insuranceSubscribeController.setSelectedPlanType('SUPER_ELITE')}"
					styleClass="filter-button" />
				<h:commandLink value="Show All"
					action="#{insuranceSubscribeController.setSelectedPlanType(null)}"
					styleClass="filter-button" />
			</div>
			...
			<h3>
				...
				<h:outputText
					value="#{insuranceSubscribeController.selectedPlanType}" />
			</h3>

			<div class="card-container">
				<h:dataTable
					value="#{insuranceSubscribeController.filteredCoverageOptions}"
					var="option">
					<h:column>
						<div class="card">
							<div class="card-columns">
								...
								<div class="card-column">
									...
									<div class="card-item">
										...
										<h:outputText
											value="#{option.insurancePlan.expireDate}" />
									</div>
									<div class="card-item">
										<span class="card-label">Waiting Month :</span>
										<h:outputText
											value="#{option.insurancePlan.waitingPeriod}" />
									</div>
								</div>
							</div>
							<div class="card-button">
								<h:commandButton value="View Details"
									action="#{insuranceSubscribeController.showPlicyDetailsToSuscribe(option.coverageId)}" />
							</div>
						</div>
